...
def adjust_confidence_with_followups(confidence, symptom_details, illness_name, user_answers):
    ...
    for symptom in symptom_details:
        symptom_name = symptom["name"].lower()  # Case-insensitive matching
        ...
        if symptom_name in symptom_followups and illness_rule:
            ...
            if symptom_name in expected_symptoms:
                ...
                expected_subtypes = [s.strip().lower() for s in expected_data.get("subtype", "Any").split(",") if s.strip()]
                ...
                # Retrieve user subtype dynamically
                user_subtype_key = next(
                    (q for q in symptom_followups[symptom_name]["questions"]
                     if any(x in q for x in ["Dry or Wet", "Watery or Bloody", "Mild or Severe"])),
                    None
                )
                user_subtype = user_response.get(user_subtype_key, None) if user_subtype_key else None

                # If user_subtype is None, check if severity response matches expected subtypes
                if not user_subtype and user_severity and user_severity.lower() in expected_subtypes:
                    user_subtype = user_severity

                # Normalize user subtype for comparison
                ...
                # KB Matching Bonus (default 1.0)
                kb_match_bonus = 1.0

                # Apply KB Matching Bonus or Penalty for Severity
                if user_severity and expected_severity != "Any":
                    kb_match_bonus *= 1.02 if user_severity.lower() == expected_severity.lower() else 0.95

                # Apply KB Matching Bonus or Penalty for Duration
                if user_duration and expected_duration != "Any":
                    kb_match_bonus *= 1.03 if user_duration in expected_duration else 0.95

                # Apply KB Matching Bonus or Penalty for Subtype
